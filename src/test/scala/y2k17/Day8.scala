...
class Day8 extends WordSpecLike with Matchers {
  ...
  def executeStatement(statement: String)(data: Map[String, Int]): Map[String, Int] = {
    val ss = statement.split(" if ")
    val modL = ss.head.split(" ")
    val predL = ss(1).split(" ")
    if(predicate(predL.head, predL(1), predL(2).toInt)(data)){
      modifier(modL.head, modL(1), modL(2).toInt)(data)
    } else {
      data
    }
  }

  def inc(name: String, value: Int)(data: Map[String, Int]): Map[String, Int] = {
    data + (name -> (data.getOrElse(name, 0) + value))
  }

  def dec(name: String, value: Int)(data: Map[String, Int]): Map[String, Int] = {
    data + (name -> (data.getOrElse(name, 0) - value))
  }

  def modifier(name: String, condition: String, value: Int)(data: Map[String, Int]): Map[String, Int] = {
    condition match {
      case "inc" => inc(name, value)(data)
      case "dec" => dec(name, value)(data)
    }
  }

  def predicate(name: String, condition: String, value: Int)(data: Map[String, Int]): Boolean = {
    val d = data.getOrElse(name, 0)
    condition match {
      case "==" => d == value
      case "!=" => d != value
      case "<=" => d <= value
  ...
  def d8p1(rawData: String): Int = {
    val registers = rawData.split("\n").toList.foldLeft(Map.empty[String, Int])((map, str) => executeStatement(str)(map))
    registers.maxBy(_._2)._2
  }

  def d8p2(rawData: String): Int = {
    val (registers, highest) = rawData.split("\n").toList.foldLeft((Map.empty[String, Int], 0)){
      case ((map, h), str) =>
        val m = executeStatement(str)(map)
        val h2 = m.maxBy(_._2)._2
        if(h2 > h) {
          (m, h2)
        } else {
          (m, h)
  ...
  "Day8" should {
    "solve the part1" in {
      println(d8p1(Day8.value))
    }

    "solve the part2" in {
      println(d8p2(Day8.value))
    }
  }
