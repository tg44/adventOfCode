...
class Day7 extends WordSpecLike with Matchers {
  ...
  case class ParsedData(name: String, weight: Int, childs: Seq[String])

  case class TreeItem(name: String, weight: Int, childs: Seq[TreeItem]) {
    lazy val subTreeWeight: Int = childs.map(_.subTreeWeight).sum + weight

    lazy val isBalanced: Boolean = if(childs.nonEmpty){
      childs.foldRight[Either[Unit, Int]](Right(childs.head.subTreeWeight))( (a,b) => if(b.isRight) if(b.getOrElse(0) == a.subTreeWeight) b else Left(()) else Left(())).isRight
    } else {
      true
    }
  }

  def parseLine(str: String): ParsedData = {
    val f = str.split(" -> ")
    val g = f.head.split(" ")
    val name = g.head
    val weight = g(1).drop(1).dropRight(1).toInt
    val childs = if(f.size > 1) {
      f(1).split(", ").toList
    } else {
      Seq()
    }
    ParsedData(name, weight, childs)
  }

  def findRoot(list: Seq[ParsedData]): ParsedData = {
    val name = list.map(_.name).filter(n => !list.exists(_.childs.contains(n))).head
    list.find(_.name == name).get
  }

  def d7p1(rawData: String): String = {
    val list = rawData.split("\n").toList
    val listPD: Seq[ParsedData] = list.map(parseLine)
    findRoot(listPD).name
  }

  def buildToTree(root: ParsedData, allNodes: Seq[ParsedData]): TreeItem = {
    TreeItem(root.name, root.weight, allNodes.filter(n => root.childs.contains(n.name)).map(buildToTree(_, allNodes)))
  }

  @tailrec
  final def findUnbalanced(root: TreeItem): TreeItem = {
    if(!root.isBalanced){
      if(root.childs.forall(_.isBalanced)) root
      else findUnbalanced(root.childs.find(!_.isBalanced).get)
    } else {
      throw new Exception()
    }
  }

  def d7p2(rawData: String): Int = {
    val list = rawData.split("\n").toList
    val listPD: Seq[ParsedData] = list.map(parseLine)
    val root = findRoot(listPD)
    val tree = buildToTree(root, listPD)
    tree.subTreeWeight
    val unbalancedRoot = findUnbalanced(tree)
    val grupped: Map[Int, Seq[TreeItem]] = unbalancedRoot.childs.groupBy(_.subTreeWeight)
    val wrong = grupped.filter(_._2.size == 1).head
    val good = grupped.filter(_._2.size != 1).head
    good._1 - wrong._2.head.childs.map(_.subTreeWeight).sum
  }

  "parseLine" must {

    "pars line without childs" in {
      parseLine("xcpuy (20)") shouldBe ParsedData("xcpuy", 20, Seq())
    }

    "parse line with one child" in {
      parseLine("pgvds (134) -> manpd") shouldBe ParsedData("pgvds", 134, Seq("manpd"))
    }

    "parse line with multiple childs" in {
      parseLine("ibjoxjo (175) -> ubhsnqw, icjmhii, lmvfuql, wrnmwy") shouldBe ParsedData("ibjoxjo", 175, Seq("ubhsnqw", "icjmhii", "lmvfuql", "wrnmwy"))
    }

  }

  "day7" should {
    "solve the problem1" in {
      println(d7p1(Day7.input))
    }
    "solve the problem2" in {
      println(d7p2(Day7.input))
    }
  }
