...
class Day9 extends WordSpecLike with Matchers {

  sealed trait State

  case object `!` extends State

  case object InStream extends State

  case object InGarbage extends State

  @tailrec
  final def tailrecStatemachine(str: List[Char], actualScore: Int, sumScore: Int, state: State): Int = {
    state match {
      case `!` =>
        //println(s"skipped $str.head")
        tailrecStatemachine(str.tail, actualScore, sumScore, InGarbage)

      case InStream =>
        //println(s"InStream ${str.mkString}, $actualScore, $sumScore")
        str match {
          case Nil => sumScore
          case head :: tail if head == ',' => tailrecStatemachine(tail, actualScore, sumScore, InStream)
          case head :: tail if head == '{' => tailrecStatemachine(tail, actualScore + 1, sumScore, InStream)
          case head :: tail if head == '}' => tailrecStatemachine(tail, actualScore - 1, sumScore + actualScore, InStream)
          case head :: tail if head == '<' => tailrecStatemachine(tail, actualScore, sumScore, InGarbage)
        }

      case InGarbage =>
        //println(s"InGarbage ${str.mkString}")
        str match {
          case Nil => sumScore
          case head :: tail if head == '!' => tailrecStatemachine(tail, actualScore, sumScore, `!`)
          case head :: tail if head == '>' => tailrecStatemachine(tail, actualScore, sumScore, InStream)
          case head :: tail => tailrecStatemachine(tail, actualScore, sumScore, InGarbage)
        }

    }
  }

  def d9p1(str: String): Int = {
    tailrecStatemachine(str.toList, 0, 0, InStream)
  }

  @tailrec
  final def tailrecStatemachine2(str: List[Char], sumRemoved: Int, state: State): Int = {
    state match {
      case `!` =>
        //println(s"skipped $str.head")
        tailrecStatemachine2(str.tail, sumRemoved, InGarbage)

      case InStream =>
        //println(s"InStream ${str.mkString}, $actualScore, $sumScore")
        str match {
          case Nil => sumRemoved
          case head :: tail if head == ',' => tailrecStatemachine2(tail, sumRemoved, InStream)
          case head :: tail if head == '{' => tailrecStatemachine2(tail, sumRemoved, InStream)
          case head :: tail if head == '}' => tailrecStatemachine2(tail, sumRemoved, InStream)
          case head :: tail if head == '<' => tailrecStatemachine2(tail, sumRemoved, InGarbage)
        }
      ...
      case InGarbage =>
        ...
        str match {
          case Nil => sumRemoved
          case head :: tail if head == '!' => tailrecStatemachine2(tail, sumRemoved, `!`)
          case head :: tail if head == '>' => tailrecStatemachine2(tail, sumRemoved, InStream)
          case head :: tail => tailrecStatemachine2(tail, sumRemoved + 1, InGarbage)
        }

    }
  }

  def d9p2(str: String): Int = {
    tailrecStatemachine2(str.toList, 0, InStream)
  }

  "tailrecStatemachine" should {
    val tests = Seq(
      ("{}", 1),
      ("{{{}}}", 6),
      ("{{},{}}", 5),
    ...
    tests.foreach {
      case (input, expected) =>
        s"return $expected for $input" in {
          tailrecStatemachine(input.toList, 0, 0, InStream) shouldBe expected
        }
    }
  }

  "tailrecStatemachine2" should {
    val tests = Seq(
      ("{}", 0),
      ("{{{}}}", 0),
      ("{{},{}}", 0),
    ...
    tests.foreach {
      case (input, expected) =>
        s"return $expected for $input" in {
          tailrecStatemachine2(input.toList, 0, InStream) shouldBe expected
        }
    }
  }
  ...
  "Day9" should {
    "solve the part1" in {
      println(d9p1(Day9.value))
    }

    "solve the part2" in {
      println(d9p2(Day9.value))
    }
  }
