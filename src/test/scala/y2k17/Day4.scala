...
class Day4 extends WordSpecLike with Matchers {
  def parse(rawData: String): Seq[Seq[String]] = {
    rawData.split("\n").toList.map(_.split("\\s+").toList.map(_.trim).filter(_!=""))
  }

  def validate(data: Seq[String]): Boolean = {
    data.size == data.toSet.size
  }

  def checkAnagrams(head: String, tail: Seq[String]): Boolean = {
    val s = head.toSet
    val optedList = tail.filter(_.length == head.length).filter(_.toSet == s)
    if(optedList.isEmpty)
      false
    else {
      head.permutations.exists(optedList.contains(_))
    }
  }

  @tailrec
  final def validate2(data: Seq[String]): Boolean = {
    data match {
      case Nil => true
      case head :: Nil => true
      case head :: tail =>
        if(checkAnagrams(head,tail)) {
          false
        } else {
          validate2(tail)
        }
  ...
  def d4p1(rawData: String): Int = {
    parse(rawData)
      .map(validate)
      .count(identity)
  }

  def d4p2(rawData: String): Int = {
    parse(rawData)
      .map(validate2)
      .count(identity)
  }
    ...
    tests.foreach{
      case (input, expected) =>
        s"return $expected for $input" in {
          validate(input.split("\\s+").toList) shouldBe expected
        }
    }
  ...
  "validate2" should {
    ...
    tests.foreach{
      case (input, expected) =>
        s"return $expected for $input" in {
          validate2(input.split("\\s+").toList) shouldBe expected
        }
    }
  ...
  "solve the given input" in {
    println(d4p1(Day4.myInput))
    println(d4p2(Day4.myInput))
  }

}

object Day4 {
  val myInput = """oaoe rxeq vssdqtu xrk cjv yaoqp loo
