...
class Day2 extends WordSpecLike with Matchers {
  ...
  val inputSheet = """4168	3925	858	2203	440	185	2886	160	1811	4272	4333	2180	174	157	361	1555
  ...
  def parse(spreadsheet: String): scala.Seq[scala.Seq[Int]] = {
    spreadsheet.split("\n").toList.map(_.split("\\s+").toList.map(_.trim).filter(_!="").map(_.toInt))
  }

  def calcChecksum(ints: Seq[Int]): Int = {
    ints.max - ints.min
  }

  def d2p1(spreadsheet: String): Int = {
    val parsed: Seq[Seq[Int]] = parse(spreadsheet)
    parsed.map(calcChecksum(_)).sum
  }

  def calcChecksum2(ints: Seq[Int]): Int = {
    val filteredList = ints.combinations(2).filter(x => x.max % x.min == 0).toList.headOption.getOrElse(throw new IllegalArgumentException("not valid row"))
    filteredList.max / filteredList.min
  }

  def d2p2(spreadsheet: String): Int = {
    val parsed: Seq[Seq[Int]] = parse(spreadsheet)
    parsed.map(calcChecksum2(_)).sum
  }
  ...
  "d2p1" should {
    ...
    "solve the example" in {
      ...
      d2p1(spreadsheet) shouldBe 18
    }

    "solve the given input" in {
      println(d2p1(inputSheet))
    }
  ...
  "d2p2" should {
    ...
    "solve the example" in {
      ...
      d2p2(spreadsheet) shouldBe 9
    }

    "solve the given input" in {
      println(d2p2(inputSheet))
    }
